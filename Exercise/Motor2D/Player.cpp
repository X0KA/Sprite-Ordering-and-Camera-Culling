...
bool Player::Start(){
	collider = App->colliders->AddCollider({ (int)original_pos.x, (int)original_pos.y, (int)(250*scale), (int)(250*scale) }, COLLIDER_PLAYER, (j1Module*)App->entities);
	win_width = App->win->screen_surface->w;
	win_height = App->win->screen_surface->h;
	win_scale = App->win->GetScale();
	order = 2;
